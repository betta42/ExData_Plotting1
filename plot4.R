...
#4
#this script:
#creates a temporary working dir "temp_4" where to download the data
#downloads, unzip and reads the data
#create plot 4 required by the assigned and
#saves it as a png file in temp_4
...
#to have 4 plots, the second parameters fixes size of legend
par(mfrow=c(2,2), cex=.48)

#4a
plot(x1$newdate, x1$Global_active_power, type = "l", ylab="Global Active Power (kilowatts)", xlab="", yaxt='n' )
axis(2, at=c(0,1000,2000,3000), labels=c(0,2,4,6))#should be the original /500, just renaming doesn't seem the smartes solution

#4b
plot(x1$newdate, x1$Voltage, type = "l", ylab="Voltage", xlab="datetime")
...
legend("topright", leg_text, col = c(1, "red", "blue"),lty = 1, bty = "n")

#4d
plot(x1$newdate, x1$Global_reactive_power, type = "l", ylab="Global_reactive_power", xlab="datetime")
